  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        <div className="flex items-start gap-3 sm:gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
              {[
                { label: "Total XP",  value: formatXP(xp),                                                icon: "⚡" },
                { label: "Volume",    value: `$${(userPoints?.total_volume_usd ?? 0).toLocaleString()}`,   icon: "📈" },
                { label: "Swaps",     value: userPoints?.swap_count ?? 0,                                  icon: "🔄" },
                { label: "Streak",    value: `${userPoints?.streak_days ?? 0}d 🔥`,                       icon: null },
              ].map((stat) => (
                <div key={stat.label} className="rounded-xl bg-bg-tertiary border border-border px-3 py-2.5">
      ...
      <div className="glass-card p-4">
        <h3 className="font-semibold text-sm mb-3 flex items-center gap-2 text-text-primary">
          📋 Recent Activity
        </h3>
        {historyLoading ? (
